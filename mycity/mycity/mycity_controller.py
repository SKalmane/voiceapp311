...
from .intents.polling_location_intent import get_polling_location_intent
...
def on_intent(mycity_request):
    ...
    elif mycity_request.intent_name == "PollingLocationIntent":
        return request_user_address_response(mycity_request) \
            if intent_constants.CURRENT_ADDRESS_KEY \
            not in mycity_request.session_attributes \
            else get_polling_location_intent(mycity_request)
